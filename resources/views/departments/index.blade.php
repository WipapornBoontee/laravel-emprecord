    ...
    <div class="col-lg-8">
        <div class="dept-card overflow-hidden">
            <div class="p-3 px-4 border-bottom border-theme d-flex flex-wrap align-items-center justify-content-between gap-3">
                ...
                <form action="{{ route('departments.index', [], false) }}" method="GET" class="d-flex flex-wrap align-items-center gap-2 m-0">
                    <div class="input-group input-group-sm" style="width: 200px;">
                        <span class="input-group-text bg-transparent border-theme text-muted"><i class="bi bi-search"></i></span>
                        <input type="text" name="search" class="form-control form-control-custom py-1" placeholder="ค้นหาแผนก..." value="{{ $search ?? '' }}">
                    </div>
                    @if(!empty($search))
                        <a href="{{ route('departments.index', ['per_page' => $perPage], false) }}" class="btn btn-sm btn-outline-secondary" title="ล้างการค้นหา">
                    ...
                    <div class="d-flex align-items-center gap-1 ms-sm-2">
                        ...
                        <select name="per_page" id="per_page_select" class="form-select form-select-sm form-control-custom py-1" style="width: 80px;" onchange="this.form.submit()">
